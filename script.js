...
let p1Stunned = false, p2Stunned = false;
...
function autoScaleMath(elementId) {
    ...
    setTimeout(() => {
        ...
        const maxWidth = parent.clientWidth - 30;
        ...
        if (mjx) {
            ...
            if (renderedWidth > maxWidth) {
                ...
                el.style.fontSize = Math.max(1.6 * ratio, 0.75) + 'rem';
            }
        }
    }, 150);
}
...
function nextRound() {
    ...
    [...currentQ.opts].sort(() => Math.random() - 0.5).forEach(o => {
        ...
        b.innerHTML = "\\(" + o + "\\)";
        b.onclick = () => {
            if (o === currentQ.a) { score++; xp += 20; history.total++; history.correct++; sessionQueue.shift(); nextRound(); }
            else { history.total++; handleFail(); }
...
// --- PVP LOGIC ---
window.startPvP = () => {
    p1Score = 0; p2Score = 0; p1Stunned = false; p2Stunned = false;
...
function renderPvP(q, pNum) {
    ...
    [...q.opts].sort(() => Math.random() - 0.5).forEach(o => {
        ...
        b.onclick = () => {
            if ((pNum === 1 && p1Stunned) || (pNum === 2 && p2Stunned)) return;
            if (o === q.a) {
...
function stun(pNum) {
    if (pNum === 1) p1Stunned = true; else p2Stunned = true;
    document.getElementById(`p${pNum}-zone`).classList.add('stunned');
    setTimeout(() => {
        if (pNum === 1) p1Stunned = false; else p2Stunned = false;
...
window.showScreen = (id) => {
    ...
    if (id === 'screen-game' || id === 'screen-pvp' || id === 'screen-login') dock.classList.add('hidden');
...
window.setDiff = (s) => { timeLimit = s; score = 0; lives = 3; sessionQueue = [...filteredQ].sort(()=>Math.random()-0.5); showScreen('screen-game'); nextRound(); };
...
function genSymbols() {
    ...
    for(let i=0; i<25; i++){
        const s = document.createElement('span'); s.className = 'float-symbol'; s.innerText = syms[Math.floor(Math.random()*syms.length)];
