...
// --- RENDERING & AGGRESSIVE SCALING ---
function autoScaleMath(elementId) {
    const el = document.getElementById(elementId);
    if (!el) return;
    
    // In PvP, use a smaller base font to give more room for long formulas
    const isPvP = elementId.includes('p1') || elementId.includes('p2');
    el.style.fontSize = isPvP ? '1.3rem' : '1.6rem';
    
    setTimeout(() => {
        const mjx = el.querySelector('mjx-container');
        if (mjx) {
            const parentWidth = el.parentElement.clientWidth - 15;
            const renderedWidth = mjx.getBoundingClientRect().width;
            if (renderedWidth > parentWidth) {
                const base = isPvP ? 1.3 : 1.6;
                el.style.fontSize = Math.max(base * (parentWidth / renderedWidth), 0.65) + 'rem';
            }
        }
    }, 150);
}
        ...
        window.MathJax.typesetPromise().then(() => {
            ...
            document.querySelectorAll('.opt-node, .opt-pvp').forEach(node => {
                ...
                if (inner) {
                    ...
                    if (curW > maxW) node.style.fontSize = Math.max(maxW/curW, 0.6) + 'rem';
                }
            });
        });
    }
...
window.setDiff = (s) => { 
    ...
    showScreen('screen-game'); nextRound(); 
};
...
function nextRound() {
    ...
    [...currentQ.opts].sort(() => Math.random() - 0.5).forEach(o => {
        const b = document.createElement('button'); b.className = 'opt-node'; b.innerHTML = "\\(" + o + "\\)";
        b.onclick = () => {
            history.total++;
...
function renderPvP(q, pNum) {
    const cont = document.getElementById(`p${pNum}-options`); cont.innerHTML = "";
    [...q.opts].sort(() => Math.random() - 0.5).forEach(o => {
        const b = document.createElement('button'); b.className = 'opt-pvp'; b.innerHTML = "\\(" + o + "\\)";
        b.onclick = () => {
...
function populateLogs() {
    const logs = Object.entries(failLogs).map(([q, c]) => `<div class="vault-item"><div>\\(${q}\\)</div><div style="color:var(--accent);margin-top:10px">Gaps: ${c}</div></div>`);
    document.getElementById('logs-list').innerHTML = logs.length ? logs.join('') : "<p style='text-align:center'>No gaps.</p>";
}
