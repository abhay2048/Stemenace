...
function nextPvPRound() {
    ...
    if (pvpQueue.length === 0) { alert("Duel Done!"); showScreen('screen-home'); return; }
...
window.startRepair = () => { filteredQ = allQ.filter(q => failLogs[q.q]); showScreen('screen-difficulty'); };

// --- HELPERS ---
function startTimer() {
    let cur = timeLimit; const bar = document.getElementById('timer-fill');
    timerId = setInterval(() => { cur -= 0.1; bar.style.width = (cur/timeLimit)*100+"%"; if(cur<=0) handleFail(); }, 100);
}
